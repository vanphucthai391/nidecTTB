...
namespace BoxIdDB
{
    public partial class frmModule0148 : Form
    {
        ...
        public int limit2 = 0;
        bool sound;
        public frmModule0148()
        {
            InitializeComponent();
            dtAllProcess = new DataTable();
        }
        private void frmModule0148_Load(object sender, EventArgs e)
        {
        ...
        public void updateControls(string frmName, string boxId, DateTime registDate, string serialNo, string invoice, string user, bool editMode, bool returnMode)
        {
            ...
            if (boxId != "")
            {
               ...
               string model = box_arr[0];
                productTable = "product_serial_0148";
                switch (model)
                {
                    case "0148":
                        cmbModel.Text = "BMA_0148";
                        limit1 = 40;
                        break;
                }
        ...
        private void defineAndReadDtOverall(ref DataTable dt)
        {
            ...
            dt.Columns.Add("terminal", Type.GetType("System.String"));

            if (!formEditMode)
            {
                string sql;
                sql = "select serialno, model, lot, inspectdate, cio_ccw, cg_ccw, cno_ccw, tjudge, date_line, aio_ccw, ano_ccw, air_ccw, anr_ccw, ais_ccw, tjudge_line, return, terminal " +
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (serial != String.Empty)
                    {
                        ...
                        // " WHERE serno = (SELECT serno from(select lot, serno,process, inspectdate, ROW_NUMBER() OVER(PARTITION BY process ORDER BY inspectdate DESC) from " + testerTableThisMonth + " where (process = 'NO41' and serno = '" + serial + "') order by serno) tbl where row_number =1) and inspect in ('AIO_CCW','AIR_CCW','AIS_CCW','ANO_CCW','ANR_CCW'))" + "a group by SERNO, INSPECTDATE , INSPECT , INSPECTDATA , JUDGE ) b where flag = 1) c," + "(select serno, tjudge from " + tableThisMonth + " where serno = (SELECT serno from(select lot, serno,process, inspectdate, ROW_NUMBER() OVER(PARTITION BY process ORDER BY inspectdate DESC) from " + testerTableThisMonth + " where (process = 'NO41' and serno = '" + serial + "') order by serno) tbl where row_number =1) and process = 'NO41' order by inspectdate desc LIMIT 1) d" +
                        // " group by d.serno, d.tjudge, c.inspectdate, c.inspect, c.inspectdata, c.judge) e " +
                        // " GROUP BY serno, tjudge, inspectdate" +
                        ...
                        // " WHERE serno = (SELECT serno from(select lot, serno,process, inspectdate, ROW_NUMBER() OVER(PARTITION BY process ORDER BY inspectdate DESC) from " + testerTableThisMonth + " where (process = 'NO41' and serno = '" + serial + "') order by serno) tbl where row_number =1) and inspect in ('AIO_CCW','AIR_CCW','AIS_CCW','ANO_CCW','ANR_CCW'))" + "a group by SERNO, INSPECTDATE , INSPECT , INSPECTDATA , JUDGE ) b where flag = 1) c," + "(select serno, tjudge from " + tableLastMonth + " where serno = (SELECT serno from(select lot, serno,process, inspectdate, ROW_NUMBER() OVER(PARTITION BY process ORDER BY inspectdate DESC) from " + testerTableThisMonth + " where (process = 'NO41' and serno = '" + serial + "') order by serno) tbl where row_number =1) and process = 'NO41' order by inspectdate desc LIMIT 1) d" +
                        // " group by d.serno, d.tjudge, c.inspectdate, c.inspect, c.inspectdata, c.judge) e " +
                        // " GROUP BY serno, tjudge, inspectdate" +
                        ...
                        string sql1 = "   select serno, tjudge as tjudge_line, inspectdate as date_line, " +
                         ...
                         "MAX(case inspect when 'AIS_CCW' then inspectdata else null end) as AIS_CCW" +
                                   " FROM" +
                                   " (select d.serno, d.tjudge, c.inspectdate, c.inspect, c.inspectdata, c.judge from (select SERNO, INSPECTDATE, INSPECT, INSPECTDATA, JUDGE from (select SERNO, INSPECT, INSPECTDATA, JUDGE, max(inspectdate) as inspectdate, row_number() OVER(PARTITION BY inspect ORDER BY max(inspectdate) desc) as flag from (select * from " + testerTableThisMonth + "data" +
                                   " WHERE serno = (SELECT serno from(select lot, serno,process, inspectdate, ROW_NUMBER() OVER(PARTITION BY process ORDER BY inspectdate DESC) from " + testerTableThisMonth + " where (process = 'NO41' and serno = '" + serial + "') order by serno) tbl where row_number =1) and inspect in ('AIO_CCW','ANO_CCW','AIR_CCW','ANR_CCW','AIS_CCW'))" + "a group by SERNO, INSPECTDATE , INSPECT , INSPECTDATA , JUDGE ) b where flag = 1) c," + "(select serno, tjudge from " + testerTableThisMonth + " where serno = '" + serial + "' and process = 'NO41' and tjudge = '0' order by inspectdate desc LIMIT 1) d" +
                                   " group by d.serno, d.tjudge, c.inspectdate, c.inspect, c.inspectdata, c.judge) e " +
                                   " GROUP BY serno, tjudge, inspectdate" +
                        ...
                        string lotthis = string.Format("SELECT serno FROM (SELECT serno, inspectdate, ROW_NUMBER() OVER(PARTITION BY lot ORDER BY inspectdate DESC) from {1} where serno = '{0}' and process = 'NO41' ORDER BY lot)tb where ROW_NUMBER = 1", serial, tableThisMonth);
                        string lotlast = string.Format("SELECT serno FROM (SELECT serno, inspectdate, ROW_NUMBER() OVER(PARTITION BY lot ORDER BY inspectdate DESC) from {1} where serno = '{0}' and process = 'NO41' ORDER BY lot)tb where ROW_NUMBER = 1", serial, tableLastMonth);
                        ...
                        string queryProcess = string.Format("SELECT serno, lot, inspectdate, process,judge from "
                            ...
                            + "WHERE serno in (SELECT DISTINCT serno FROM {1} WHERE process = 'NO41' AND serno = '{0}') "
                            + "OR serno = '{0}' "
                            ...
                            + "UNION ALL SELECT DISTINCT '{0}' as serno, serno lot,inspectdate ,process,"
                            + "(CASE WHEN tjudge = '0' THEN 'PASS' ELSE 'FAILURE' END) AS judge FROM {2} "
                            + "WHERE serno in (SELECT DISTINCT serno FROM {2} WHERE process = 'NO41' AND serno = '{0}') "
        ...
        private void ShowProcessJudge(string serial)
        {
            if (dtAllProcess.Rows.Count > 0)
            {
                ...
                var checknmt4 = dtAllProcess.AsEnumerable().Any(x => x.Field<string>("process").Contains("NMT3"));
                ...
                var checkHC = dtAllProcess.AsEnumerable().Any(x => x.Field<string>("process").Contains("HEIGHTCASE"));
                ...
                if (!checkFail && checknmt4 && checkno41 && checkno43 && checkHC && checkno47 && checkno48)
                {
                    txtCount.Text = "OK";
        ...
        private void btnRegisterBoxId_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrEmpty(txtCarton.Text))
            {
                ...
                TfSQL tf = new TfSQL();
                bool res1 = tf.sqlMultipleInsert0148(dt);

                if (res1)
                {
                    string shipKind = dtOverall.Rows[0]["return"].ToString();
                   // string prt_model = cmbModel.Text.Substring(0, 4) + cmbModel.Text.Substring(4);
                    dtOverall.Clear();
                    dt = null;
